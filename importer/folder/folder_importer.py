...
class FolderImporter(Importer):
    ...
    def fetch_dashboards(self):
        ...
        if os.path.isdir(self._path):
            for filename in [
                file for file in os.listdir(self._path) if file.endswith(".json")
            ]:
                try:
                    ...
                    super()._logger.error(
                        f"Failed opening file: {filename}, error: {str(e)}"
                    )
                    continue
        else:
            super()._logger.error(
                f"Invalid folder path. Could not extract dashboards from: {self._path}"
            )
        return dashboards
